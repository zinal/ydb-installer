# YDB Installer Functional Specification

## 1. Document Control

### 1.1 Purpose

This document defines the functional specification for a web-based YDB installer application supporting interactive and batch installation of YDB clusters on bare-metal or virtual-machine infrastructure.
...
- `Static Node`: a YDB storage node.
- `Dynamic Node`: a YDB database-serving node.
...
4. configuration input or import;
5. review and approval;
6. host preparation;
7. artifact and certificate preparation;
8. static node installation and startup;
9. storage initialization;
10. database creation;
11. dynamic node installation and startup;
...
FR-010. The Installer shall not permit disk formatting, repartitioning, or storage initialization until discovery and validation have completed successfully.
...
FR-032. The Installer shall validate the batch specification before starting any destructive action.
...
FR-043. The Installer shall support assignment of hosts as static nodes, dynamic nodes, or both where allowed by the configuration.

FR-044. The Installer shall support configuration of one or more dynamic nodes per host.
...
FR-074. The Installer shall perform preflight validation before starting installation execution.

FR-075. The Installer shall classify validation results as:

- blocking error;
- warning;
- informational notice.

FR-076. The Installer shall present validation results per host and for the overall cluster configuration.

### 12.2 Required Checks

FR-077. The preflight validation shall include at minimum checks for:
...
FR-078. The Installer shall validate that bridge mode, when selected, is configured with at least two piles and that each pile has the required host assignments.

FR-079. The Installer shall warn if the planned cluster layout does not satisfy documented production recommendations for the selected base storage topology or bridge layout.

FR-080. The Installer shall prevent execution from proceeding while blocking errors remain unresolved.

## 13. Execution Engine Behavior

### 13.1 Host Preparation

FR-081. The Installer shall prepare hosts according to the selected installation plan, including prerequisite packages, system settings, and YDB runtime user preparation.

FR-082. The Installer shall copy required binaries, libraries, configuration files, and certificates to the appropriate target hosts.

### 13.2 Static Nodes

FR-083. The Installer shall install and start static nodes before attempting storage initialization.

FR-084. The Installer shall wait for static nodes to reach the required readiness level before continuing.

### 13.3 Storage Initialization

FR-085. The Installer shall initialize YDB storage only after static-node readiness checks have passed.

FR-086. The Installer shall persist the result of storage initialization and expose any failure details in the UI.

FR-087. When bridge mode is enabled, the Installer shall execute bridge-specific configuration steps in an order consistent with the selected base storage topology and pile layout.

FR-088. When bridge mode is enabled, the Installer shall expose bridge-specific progress and failure details separately from ordinary storage-initialization steps.

### 13.4 Database and Dynamic Nodes

FR-089. The Installer shall create the configured initial database before starting dynamic nodes that serve it.

FR-090. The Installer shall install and start dynamic nodes after successful database creation.

FR-091. The Installer shall support creation and activation of more than one dynamic-node service per host where configured.

### 13.5 Verification

FR-092. The Installer shall perform post-install verification at minimum by checking:
...
FR-093. When bridge mode is enabled, the Installer shall include verification of bridge-related configuration status and pile membership visibility where such checks are supported by the backend.

FR-094. The Installer shall mark the installation session as completed only if required verification checks succeed or if the operator explicitly accepts a degraded completion state.

## 14. Progress Monitoring

FR-095. The Installer shall expose real-time or near-real-time execution progress in the web UI.

FR-096. The progress view shall show:
...
FR-097. For bridge-mode installations, the progress view shall distinguish between base storage topology steps and bridge-specific steps.

FR-098. The Installer shall preserve historical logs and status transitions for later review.

## 15. Cancellation, Failure Handling, and Resume

### 15.1 Cancellation

FR-099. The Installer shall allow an authorized operator to request cancellation of a running session from the web UI.

FR-100. Cancellation shall be cooperative and phase-aware.

FR-101. If cancellation is requested before a destructive step begins, the Installer shall stop before performing that step.

FR-102. If cancellation is requested during an atomic remote action, the Installer shall complete that atomic action before stopping, unless the backend explicitly supports safe interruption.

FR-103. The Installer shall mark the session state as `cancel requested` until execution reaches a safe stop point.

### 15.2 Failure Reporting

FR-104. On failure, the Installer shall report:
...
FR-105. For bridge-mode installations, failure reporting shall identify the affected pile or piles where applicable.

### 15.3 Resume

FR-106. The Installer shall support rerunning or resuming from a safe checkpoint where the backend and session state make this possible.

FR-107. The Installer shall not claim rollback or resume capability for a phase unless that behavior is actually supported.

## 16. Reporting and Output Artifacts

FR-108. The Installer shall persist the effective installation configuration used for execution.

FR-109. The Installer shall persist the discovery snapshot, validation results, execution log, and final status report for each session.

FR-110. On successful completion, the Installer shall produce a completion report containing at minimum:
...
FR-111. When bridge mode is enabled, the completion report shall include the selected base storage topology, pile definitions, and bridge-specific verification summary.

FR-112. The Installer shall allow export of the effective installation specification for reuse in batch mode.

## 17. User Interface Requirements

FR-113. The Installer UI shall be web-based.

FR-114. The Installer UI shall support execution control, configuration input, validation feedback, and monitoring from a single consistent interface.

FR-115. The UI shall present destructive actions with explicit warning language and confirmation controls.

FR-116. The UI shall distinguish clearly between required settings, recommended settings, and advanced settings.

FR-117. The UI shall support read-only monitoring views for observers.

FR-118. The UI shall provide filtering or grouping by host, phase, and severity in progress and log views.

FR-119. The UI shall use distinct terminology for:
...
FR-120. When bridge mode is enabled, the UI shall present pile-related settings and statuses explicitly rather than folding them into generic multi-data-center wording.

## 18. Usability and Installer Best Practices

FR-121. The Installer shall default to safe values where YDB documentation provides clear recommended defaults.

FR-122. The Installer shall follow the principle `discover first, validate early, destroy late`.

FR-123. The Installer shall support a validation-only or dry-run mode that performs discovery and preflight checks without changing target systems.

FR-124. The Installer shall autosave configuration drafts in interactive mode.

FR-125. The Installer shall minimize secret exposure in UI, logs, exports, and troubleshooting output.

FR-126. The Installer shall explain blocking conditions and destructive risks in operator-oriented language rather than backend-specific jargon alone.
...
4. install a YDB cluster in phased form with progress visibility;
5. support `block-4-2`, `mirror-3-dc`, `reduced mirror-3-dc`, and bridge-enabled cluster layouts;
6. protect against accidental disk destruction through explicit discovery-based confirmation;
7. support cancellation and clear failure reporting;
8. produce reusable artifacts and a final installation report.
