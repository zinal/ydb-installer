...
FR-012. The Installer shall support target access through direct SSH and bastion-based SSH. SSH authentication should support both password-based and key-based access, which should be configurable for all hosts overall (global settings) and allow per-host overrides (per-host optional settings).
...
FR-015. The Installer shall persist the discovery results as a Discovery Snapshot tied to the installation session.
...
FR-054. For selected unpartitioned disks, the Installer shall offer a partitioning scheme suitable for YDB use before storage preparation begins.

FR-055. For selected unpartitioned disks, the Installer shall normally suggest creating one YDB partition per disk.

FR-056. If the selected base storage topology is `reduced mirror-3-dc` and fewer than three YDB disks are available on a host, the Installer shall suggest creating three YDB partitions per disk on that host.

FR-057. The Installer shall support per-disk or per-partition labeling required for YDB configuration generation.

FR-058. The Installer shall support specifying the effective disk type classification required by YDB configuration.

### 9.2 Storage Safety

FR-059. The Installer shall detect and warn about selected disks that appear to be:
...
FR-060. The Installer shall clearly distinguish between:
...
FR-061. The Installer shall require explicit operator approval before any operation that can erase data.

FR-062. The approval step shall identify exactly which hosts, disks, and planned partitions will be affected.

## 10. Security and Secrets

### 10.1 TLS Support

FR-063. The Installer shall support operator-provided TLS materials.

FR-064. The Installer shall support installer-managed certificate generation where that behavior is supported by the chosen backend workflow.

FR-065. The Installer shall validate that provided TLS assets are structurally complete for the selected cluster layout and network model.

### 10.2 Authentication

FR-066. The Installer shall support installation with YDB user authentication enabled.

FR-067. The Installer shall support capturing or importing the initial administrator credentials required for installation.

FR-068. The Installer shall prevent secrets from being displayed in clear text in routine logs and progress views.

### 10.3 Secret Storage

FR-069. The Installer shall store secrets securely and separately from general session metadata.

FR-070. The Installer shall restrict secret visibility to authorized users only.

## 11. Artifact and Version Management

FR-071. The Installer shall support the following artifact-source modes:

- download by YDB version;
- local archive;
- local binaries.

FR-072. The Installer shall validate that the chosen artifact source is consistent with the selected installation scenario.

FR-073. The Installer shall allow offline or isolated installation if all required artifacts are supplied.

FR-074. The Installer shall support selection of YDB versions compatible with either configuration V1 or configuration V2.

FR-075. The Installer shall detect or derive the effective configuration generation where possible and surface it to the operator.

FR-076. The Installer shall validate whether the selected YDB edition and version support the requested cluster layout, including bridge mode where applicable.

## 12. Validation and Preflight Checks

### 12.1 General Validation

FR-077. The Installer shall perform preflight validation after configuration input or import and before review and approval for execution.

FR-078. The preflight validation shall validate both:
...
FR-079. The Installer shall classify validation results as:

- blocking error;
- warning;
- informational notice.

FR-080. The Installer shall present validation results per host and for the overall cluster configuration.

### 12.2 Required Checks

FR-081. The preflight validation shall include at minimum checks for:
...
FR-082. The Installer shall validate that bridge mode, when selected, is configured with at least two piles and that each pile has the required host assignments.

FR-083. The Installer shall warn if the planned cluster layout does not satisfy documented production recommendations for the selected base storage topology or bridge layout.

FR-084. The Installer shall prevent execution from proceeding while blocking errors remain unresolved.

## 13. Execution Engine Behavior

### 13.1 Host Preparation

FR-085. The Installer shall prepare hosts according to the selected installation plan, including prerequisite packages, system settings, and YDB runtime user preparation.

FR-086. The Installer shall copy required binaries, libraries, configuration files, and certificates to the appropriate target hosts.

### 13.2 Storage Nodes

FR-087. The Installer shall install and start Storage Nodes before attempting storage initialization.

FR-088. The Installer shall wait for Storage Nodes to reach the required readiness level before continuing.

### 13.3 Storage Initialization

FR-089. The Installer shall initialize YDB storage only after Storage Node readiness checks have passed.

FR-090. The Installer shall persist the result of storage initialization and expose any failure details in the UI.

FR-091. When bridge mode is enabled, the Installer shall execute bridge-specific configuration steps in an order consistent with the selected base storage topology and pile layout.

FR-092. When bridge mode is enabled, the Installer shall expose bridge-specific progress and failure details separately from ordinary storage-initialization steps.

### 13.4 Database and Compute Nodes

FR-093. The Installer shall create the configured initial database before starting Compute Nodes that serve it.

FR-094. The Installer shall install and start Compute Nodes after successful database creation.

FR-095. The Installer shall support creation and activation of more than one Compute Node service per host where configured.

### 13.5 Verification

FR-096. The Installer shall perform post-install verification at minimum by checking:
...
FR-097. When bridge mode is enabled, the Installer shall include verification of bridge-related configuration status and pile membership visibility where such checks are supported by the backend.

FR-098. The Installer shall mark the installation session as completed only if required verification checks succeed or if the operator explicitly accepts a degraded completion state.

## 14. Progress Monitoring

FR-099. The Installer shall expose real-time or near-real-time execution progress in the web UI.

FR-100. The progress view shall show:
...
FR-101. For bridge-mode installations, the progress view shall distinguish between base storage topology steps and bridge-specific steps.

FR-102. The Installer shall preserve historical logs and status transitions for later review.

## 15. Cancellation, Failure Handling, and Resume

### 15.1 Cancellation

FR-103. The Installer shall allow an authorized operator to request cancellation of a running session from the web UI.

FR-104. Cancellation shall be cooperative and phase-aware.

FR-105. If cancellation is requested before a destructive step begins, the Installer shall stop before performing that step.

FR-106. If cancellation is requested during an atomic remote action, the Installer shall complete that atomic action before stopping, unless the backend explicitly supports safe interruption.

FR-107. The Installer shall mark the session state as `cancel requested` until execution reaches a safe stop point.

### 15.2 Failure Reporting

FR-108. On failure, the Installer shall report:
...
FR-109. For bridge-mode installations, failure reporting shall identify the affected pile or piles where applicable.

### 15.3 Resume

FR-110. The Installer shall support rerunning or resuming from a safe checkpoint where the backend and session state make this possible.

FR-111. The Installer shall not claim rollback or resume capability for a phase unless that behavior is actually supported.

## 16. Reporting and Output Artifacts

FR-112. The Installer shall persist the effective installation configuration used for execution.

FR-113. The Installer shall persist the discovery snapshot, validation results, execution log, and final status report for each session.

FR-114. On successful completion, the Installer shall produce a completion report containing at minimum:
...
FR-115. When bridge mode is enabled, the completion report shall include the selected base storage topology, pile definitions, and bridge-specific verification summary.

FR-116. The Installer shall allow export of the effective installation specification for reuse in batch mode.

## 17. REST API Requirements

FR-117. The Installer shall expose a documented REST API for all its externally usable functions.

FR-118. The REST API shall cover at minimum:
...
FR-119. The REST API shall be documented in a form suitable for client generation and external integration.

FR-120. The Installer UI shall use the documented REST API to retrieve data and execute all required operations, and shall not rely on separate private backend-only interfaces for normal application behavior.

## 18. User Interface Requirements

FR-121. The Installer UI shall be web-based.

FR-122. The Installer UI shall support execution control, configuration input, validation feedback, and monitoring from a single consistent interface.

FR-123. The UI shall present destructive actions with explicit warning language and confirmation controls.

FR-124. The UI shall distinguish clearly between required settings, recommended settings, and advanced settings.

FR-125. The UI shall support read-only monitoring views for observers.

FR-126. The UI shall provide filtering or grouping by host, phase, and severity in progress and log views.

FR-127. The UI shall use distinct terminology for:
...
FR-128. When bridge mode is enabled, the UI shall present pile-related settings and statuses explicitly rather than folding them into generic multi-data-center wording.

## 19. Usability and Installer Best Practices

FR-129. The Installer shall default to safe values where YDB documentation provides clear recommended defaults.

FR-130. The Installer shall follow the principle `discover first, validate early, destroy late`.

FR-131. The Installer shall support a validation-only or dry-run mode that performs discovery and preflight checks without changing target systems.

FR-132. The Installer shall autosave configuration drafts in interactive mode.

FR-133. The Installer shall minimize secret exposure in UI, logs, exports, and troubleshooting output.

FR-134. The Installer shall explain blocking conditions and destructive risks in operator-oriented language rather than backend-specific jargon alone.
